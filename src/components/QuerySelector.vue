    ...
    <el-card>
      ...
      <el-row v-for="(q,i) in queries" :gutter="20">
        ...
        <el-col :span="11" align="middle" class="slider">
          <el-input-number v-model="q.min_num" :controls="false" size="mini" :min="q.con==='level'?0:limits[q.val][0]" :max="q.con==='level'?1:limits[q.val][1]"
                           :step="0.01" :precision="2" @change="numberChange(i)"></el-input-number>
          <el-slider class="slider_color" style="width: 60%; float: left;" v-model="q.value" range :min="q.con==='level'?0:limits[q.val][0]" :max="q.con==='level'?1:limits[q.val][1]"
                     :step="0.01" @change="sliderChange(i)"></el-slider>
          <el-input-number v-model="q.max_num" :controls="false" size="mini" :min="q.con==='level'?0:limits[q.val][0]" :max="q.con==='level'?1:limits[q.val][1]"
                           :step="0.01" :precision="2" @change="numberChange(i)"></el-input-number>
...
<span>
  <el-input-number v-model="q.years" :min="1" :max="10" :precision="0" size="mini"></el-input-number><span
    style="font-size: 13px;">年</span>
</span>
        </el-col>
...
<span class="button">
  <el-button class="buttons" type="primary" :disabled="this.queries.length===0" @click="reset"
